...
from Xinshuo_PyToolbox.xinshuo_io.file_io import mkdir_if_missing, load_list_from_folder, fileparts
...
from AB3DMOT_libs.nuScenes2KITTI_helper import create_KITTI_transform, convert_anno_to_KITTI, save_image, save_lidar, save_radar
...
class KittiConverter:
    def __init__(self,
        ...
        self.split = split
        if split in ['train', 'val', 'trainval','mini_train','mini_val']: self.nusc_version = 'v1.0-trainval'
        ...
        for scene_name in scene_names:
            ...
            for sample_token in sample_tokens:
                ...
                pose_record, cs_record_lid, cs_record_cam, filename_lid_full, filename_cam_full = \
                    get_sensor_param(self.nusc, sample_token, lidar_name=self.lidar_name, cam_name=self.cam_name, output_file=True)
                ...
                save_radar(self.nusc, filename_lid_full, lidar_dir, frame_id)
    ...
    def nuscenes_obj_result2kitti(self):
        ...
        with open(result_file) as json_file:
            ...
            for sample_token, dets in data['results'].items():
                ...
                pose_record, cs_record_lid, cs_record_cam = get_sensor_param(self.nusc, sample_token, cam_name=self.cam_name, lidar_name=self.lidar_name)
                velo_to_cam_trans, velo_to_cam_rot, r0_rect, p_left_kitti = \
                    nuScenes_transform2KITTI(cs_record_lid, cs_record_cam)
